...
exports.RooWifi = function(options, cb) {
  ...
  defaults = { portno      : 9001
             ...
             , _write      : write
             , commands    : commands.sets.RooWiFi
             };
...
// with the RooWiFi, you can't get sensor data if the command connection is doing something... (sigh)
// states: quiescentP - no commands are being sent, so HTTP may go active
//         busyP      - HTTP is active, so queue any packets for the command connection
//         queueP     - commands are being queued

var write = function(robot, packet) {
  var retry;

  if ((!robot.busyP) && (!robot.queueP)) {
    delete(robot.quiescentP);

    if (robot.options.debug) {
    ...
    if ((!!robot.sendQ) && (robot.sendQ.length > 0)) {
      delete(robot.timer);
      return robot;
    }
    robot.timer = setTimeout(function() {
      robot.quiescentP = true;
      sensorfetch(robot);
    }, 500);
  ...
  if (robot.options.debug) console.log('busyP=' + (!!robot.busyP) + ' queueP=' + (!!robot.queueP));
  if (!!robot.queue) {
    robot.queue.push(packet);
    return robot;
  }
  robot.queueP = true;
  robot.queue = [ packet ];
  ...
  retry = function() {
    ...
    delete(robot.queueP);
    while (robot.queue.length > 0) write(robot, robot.queue.shift());
    delete(robot.queue);
  };
  setTimeout(retry, 0);
