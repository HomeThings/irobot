...
var Robot = function (device, options) {
  ...
  // given either an open socket or a device path
  if (!!this.options.socket) {
    var self = this;

    self.serial = self.options.socket;
    self.serial.setNoDelay();
    self.serial.on('data', function(data) {
      self._parseSerialData(self.serial, data);
    }).on('end', function() {
      self.emit('end');
...
Robot.prototype._init = function () {
  ...
  this._sendCommand(this.commands.Start || this.commands.Control);

  // enter safe mode by default
  this.safeMode();

  if (!!this.commands.Stream) {
    ...
    this._sendCommand(this.commands.Stream, packets.length, packets);
  } else if (!!this.commands.Sensors) {
    this._sendCommand(this.commands.Sensors, 0);
  }
...
Robot.prototype._sendCommand = function (command) {
  ...
  if (!!this.options._write) return this.options._write(this, packetBytes);
  this.serial.write(packetBytes);
  if (typeof this.serial.flush === 'function') this.serial.flush();
...
Robot.prototype._storeAndPlaySong = function (notes) {
  var slot = 0;
  this._sendCommand(this.commands.Song, slot, notes.length, notes);
  this._sendCommand(this.commands.PlaySong, slot);
};
...
Robot.prototype.safeMode = function () {
  this._sendCommand(this.commands.Safe);
...
Robot.prototype.fullMode = function () {
  this._sendCommand(this.commands.Full);
  this.setLEDs();
  return this;
};
...
Robot.prototype.demo = function (demoId) {
  if (!!this.commands.Demo) return this._sendCommand(this.commands.Demo, demoId.id);

// not sure why the extra 00 for Cover and Spot commands, but the RooWifi iOS app sends them...
  if (demoId.id === demos.Cover.id) return this._sendCommand(this.commands.Cover, 0);
  if (demoId.id === demos.SpotCover.id) return this._sendCommand(this.commands.Spot, 0);
  if (demoId.id === demos.CoverAndDock.id) return this._sendCommand(this.commands.ForceSeekingDock);
  if (demoId.id === demos.Abort.id) {
    var self = this;
    setTimeout(function() { self._sendCommand(self.commands.ForceSeekingDock); }, 500);
    return this._sendCommand(this.commands.ForceSeekingDock);
  }

  throw new Error('robot does not support Demo command');
};

// tell the robot to seek out and mate with its dock. to cancel the docking
// maneuver, use #halt().
Robot.prototype.dock = function () {
  return this.demo(demos.CoverAndDock);
};
...
Robot.prototype.setLEDs = function (leds) {
  ...
  this._sendCommand(this.commands.LEDs,
...
Robot.prototype.drive = function (velocity, radius) {
  ...
  } else if (!!this.commands.DriveDirect) {
    command = this.commands.DriveDirect;
  ...
  } else throw new Error('robot does not support left/right velocity');
...
Robot.prototype.motors = function(motors) {
  return this._sendCommand(this.commands.Motors, motors & 0xff);
};
